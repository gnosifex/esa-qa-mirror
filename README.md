...
`.github/workflows/mirror.yml` runs daily and commits new/changed records. Each run's diff **is** your "what's new" report. Any adapter error — or a failed register query — turns the run red (the successful part is still committed); the per-source counts land in the job summary.

**Runs are incremental and state-driven** (`incremental:` in `config.yaml`). Every run enumerates the register and the EBA listing pages *in full* — presence/absence, delisting, archive resolution and the count checks happen daily. Detail pages are only fetched for (a) new or reappearing records, (b) anything published in the last `window_days` (via the EBA date facet / the register's publication dates), and (c) the records whose content was verified longest ago, oldest first, up to `daily_verify_budget` — so every record is re-verified within `verify_after_days`, and a missed day heals itself because the queue is driven by per-record verification age, not by the calendar. A **full sweep** (every detail page in one pass) runs whenever the last completed one is older than `sweep_after_days`; a failed sweep is simply still due and retries on the next run. The search page shows the resulting guarantees: presence checked daily, oldest content verification, date of the last full sweep.

**Every full run appends one JSON line to `runs.jsonl`** (UTC timestamp, mode — incremental or sweep —, per-source counts incl. how many records were content-checked, SHA-256 over `state.json`) and commits it — so *"checked and unchanged"* is distinguishable from *"never ran"* in the git history itself, independent of GitHub's log retention. `--limit` smoke runs don't write it.
